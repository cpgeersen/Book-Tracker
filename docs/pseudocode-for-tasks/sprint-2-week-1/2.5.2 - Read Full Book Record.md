...
read_full_book_by_isbn(isbn): -> JSON
...
read_full_book_by_title(title): -> JSON
    ...
    isbn_results = read_isbn_by_title(title) -> JSON{isbn}
...
````
read_full_book_by_author(author_last_name, author_first_name=None): -> JSON
    # First get the ISBNs associated with author
    # Author last name is more important for a search, make first optional
    isbn_results = read_isbn_by_author(author_last_name, author_first_name=None) -> JSON
...
Determined that we will not need this. Filtering will be done at the frontend based on already given results.
For example, lets say we search for books by Clive Barker that we own. We return all Clive Barker books in database
then we filter on a tag on the frontend and only show what is wanted. This allows us to dynamically filter a list
of results using multiple filters without having to query the database again since the JSON does not change.
