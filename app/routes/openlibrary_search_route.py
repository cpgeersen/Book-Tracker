    ...
    def openlibrary_search():
        ...
        if request.method == 'GET':
            ...
            if search_type == 'isbn':
                ...
                if len(isbn) == 0:
                    return render_template(ol_search_page, user_settings=user_settings_values), 200
                elif len(isbn) != 10 and len(isbn) != 13 or not str(isbn).isnumeric():
                    # Tell user that isbn must be either 10 or 13 integers
                    return render_template(ol_search_page_modal, user_settings=user_settings_values), 200

                isbn_dict = {"ISBN": isbn}
                response = read(isbn_dict, 'ol-book-isbn')

                return render_template(ol_search_page, books=response, search_type='isbn',
                ...
                return render_template(ol_search_page, books=response, search_type='title',
                                       user_settings=user_settings_values), 200
                ...
                return render_template(ol_search_page, books=response, search_type='author',
                                       user_settings=user_settings_values), 200

            # GET that calls the Purge Cache function
            if request.args.get('Purge_Cache') is not None:
                response = delete({}, 'cache')
                return render_template(ol_search_cache_modal, purge_status=response,
                                       user_settings=user_settings_values), 200
