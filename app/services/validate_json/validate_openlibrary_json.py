...
def validate_isbn_search(json_input):
    ...
    json_output['Author_1_OLID'] = json_input.get('Author_1_OLID')

    if json_input['Author_2'] is not None:
        json_output['Author_First_Name_2'] = str(json_input.get('Author_2')).split(' ')[:-1]
        json_output['Author_Last_Name_2'] = str(json_input.get('Author_2')).split(' ')[-1]
        json_output ['Author_2_OLID'] = json_input.get('Author_2_OLID')
