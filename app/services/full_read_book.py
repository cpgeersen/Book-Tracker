    ...
    try:
        ...
        if result:
            ...
            for id in converted_list_result:
                convert_to_dict["Author_IDs"].append(id)
                count += 1
                if count > 2:
                    break
...
def read_tag_table(tag_ID):
    try:
        ...
        if result:
            ...
            if convert_to_dict["Personal_Or_Academic"] == 0:
                convert_to_dict["Personal_Or_Academic"] = "Personal"
            else:
                convert_to_dict["Personal_Or_Academic"] = "Academic"
