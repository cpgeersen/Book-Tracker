...
from app.services.Book.BookUpdate import update_read_chapters
...
def test_update_read_chapters_success():
    new_chapters_completed = '0'
    create_book(book)

    response = update_read_chapters(book['ISBN'], new_chapters_completed)
    book_chapters_updated = json.loads(read_book(book['ISBN']))
    assert response[1] == 200
    assert book_chapters_updated['Chapters_Completed'] == new_chapters_completed


def test_update_read_chapters_failure():
    new_chapters_completed = '0'
    response = update_read_chapters(book['ISBN'], new_chapters_completed)
