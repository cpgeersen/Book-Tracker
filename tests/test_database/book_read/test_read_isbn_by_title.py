...
from app.services.Book.BookRead import read_isbn_by_title
...
def test_read_isbn_by_title_success():
    create_book(book_1)
    create_book(book_2)
    response = json.loads(read_isbn_by_title(book_1['Title']))
    assert response['ISBN'] == [book_1['ISBN'], book_2['ISBN']]

def test_read_isbn_by_title_failure():
    response = json.loads(read_isbn_by_title(book_1['Title']))
    assert response['Error'] == 'Title not found'

def test_read_isbn_by_title():
    create_book(book_1)
    response = json.loads(read_isbn_by_title(' Thief '))
