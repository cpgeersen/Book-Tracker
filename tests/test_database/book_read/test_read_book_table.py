...
from app.services.Book.BookRead import read_book_table
...
def test_read_book_table_success():
    ...
    assert response['Summary'] == book['Summary']
    assert response['Chapters'] == book['Chapters']
    assert response['Chapters_Completed'] == book['Chapters_Completed']
    assert response['Cover_Image'] == book['Cover_Image']

def test_read_book_table_failure():
    response = json.loads(read_book_table(book['ISBN']))
