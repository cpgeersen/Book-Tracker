...
from app.services.Book.BookRead import read_author_id
...
def test_read_author_id_failure():
    response = json.loads(read_author_id(book['ISBN']))
    assert response['Error'] == "ISBN not found"

def test_read_author_id_success():
    create_book(book)
    response = json.loads(read_author_id(book['ISBN']))
    assert response['Author_IDs'] == [1]

book_two_authors = {"ISBN": "0061091464",
          "Title": "The Thief of Always",
          "Publish_Year": "1993",
          "Summary": "After a mysterious stranger promises to end"
                     " his boredom with a trip to the magical Holiday"
...
def test_read_author_id_multiple_authors():
    create_book(book_two_authors)
    response = json.loads(read_author_id(book_two_authors['ISBN']))
    assert response['Author_IDs'] == [1, 2]
